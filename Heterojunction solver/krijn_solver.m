...
function update_calc(mat1, mat2, ax, slid_x_sub, slid_y_sub, slid_x_QW, slid_y_QW, text_sub_prop, text_QW_prop, text_length, text_elec_output, text_hole_output)
    ...
    if (Ev_QW>Ev_sub)&&(Ec_QW<Ec_sub)
        if (isreal(elec_E_next)&&isreal(hole_E_next))
            output_ev=(elec_E_next/(1.602176634*10^-19))+(hole_E_next/(1.602176634*10^-19))+Ec_QW-Ev_QW;
        elseif isreal(elec_E_next)
            output_ev=(elec_E_next/(1.602176634*10^-19))+Ec_QW-Ev_QW;
        elseif isreal(hole_E_next)
            output_ev=(hole_E_next/(1.602176634*10^-19))+Ec_QW-Ev_QW;
        else
            output_ev=Ec_QW-Ev_QW;
        end
    elseif (Ev_QW>Ev_sub)&&(Ec_QW>Ec_sub)
        if isreal(hole_E_next)
            output_ev=(hole_E_next/(1.602176634*10^-19))+Ec_sub-Ev_QW;
        else
            output_ev=Ec_sub-Ev_QW;
        end
    else
        output_ev=0;
    end
