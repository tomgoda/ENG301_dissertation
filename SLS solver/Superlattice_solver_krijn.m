...
%[Ev_sub, Ec_sub, Ev_QW, Ec_QW, hole_mass_QW, elec_mass_QW, hole_mass_sub, elec_mass_sub]
...
averaged_elec_mass = ((length_QW*number_of_wells*QW_strained(6))+(length_sub*(number_of_wells+1)*QW_strained(8)))/((length_QW*number_of_wells)+(length_sub*(number_of_wells+1)));
averaged_hole_mass = ((length_QW*number_of_wells*QW_strained(5))+(length_sub*(number_of_wells+1)*QW_strained(7)))/((length_QW*number_of_wells)+(length_sub*(number_of_wells+1)));
total_superlattice_length = (number_of_wells*length_QW)+((number_of_wells+1)*length_sub);

elec_U = (-1*sub_strained(4)+sub_strained(2));
hole_U = (-1*sub_strained(3)+sub_strained(1));
...
band_gap=(-1*sub_strained(1)+sub_strained(4)+superlattice_hole_groundstate+superlattice_elec_groundstate);
text_output.Value = {'% --- --- --- %', ...
                     ['QW valence - ', num2str(sub_strained(1)), ' eV'], ...
                     ['QW conduction - ', num2str(sub_strained(2)), ' eV'], ...
