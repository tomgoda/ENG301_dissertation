...
elec_U = -1*(-1*QW_strained(2)+QW_strained(4));
hole_U = -1*(-1*QW_strained(1)+QW_strained(3));
...
band_gap=-1*((QW_strained(1)+superlattice_hole_groundstate)-(QW_strained(4)+superlattice_elec_groundstate));
text_output.Value = {'% --- --- --- %', ...
                     ['QW valence - ', num2str(QW_strained(3)), ' eV'], ...
                     ['QW conduction - ', num2str(QW_strained(4)), ' eV'], ...
                     ['Barrier valence - ', num2str(QW_strained(1)), ' eV'], ...
                     ['Barrier conduction - ', num2str(QW_strained(2)), ' eV'], ...
                     '% --- --- --- %', ...
...
sub_v = sym(QW_strained(1));
sub_c = sym(QW_strained(2));
...
hold(ax, 'on')
     ...
     fplot(ax, sub_v-hole_ground, [0 length_sub*5+length_QW*2])
     fplot(ax, QW_c+elec_ground, [0 length_sub*5+length_QW*2])
hold(ax, 'off')
end
